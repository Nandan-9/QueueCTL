...
// RetryDeadJob moves a job from dead_jobs back to active jobs table
func RetryDeadJob(db *sql.DB, deadJobID int64) (*job.Job, error) {
    // 1. Fetch dead job
    row := db.QueryRow(`SELECT id, orig_id, command, attempts, max_retries, created_at, last_error
                        FROM dead_jobs WHERE id=?`, deadJobID)
    ...
    if err := row.Scan(&d.ID, &d.OrigID, &d.Command, &d.Attempts, &d.MaxRetries, &d.CreatedAt, &d.LastError); err != nil {
        return nil, err
    }
    ...
    id, err := InsertJob(db, j)
    if err != nil {
        return nil, err
    }
    j.ID = id

    // 4. Delete from dead_jobs table
    _, err = db.Exec(`DELETE FROM dead_jobs WHERE id=?`, deadJobID)
    if err != nil {
        return nil, err
    }
